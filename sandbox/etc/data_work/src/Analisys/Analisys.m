...
global A;
[filename, pathname] = uigetfile('*.dat', 'Выбор экспорта данных');
...
if (tmp == 1)
    ...
    set(handles.text11,'String','Рассчитанная модель:');
    ...
    set(handles.edit16,'String',0);
    set(handles.edit17,'String',0);
    set(handles.text11,'String','Введите модель:        ');
...
if (get(handles.type_mdl,'Value') == 1)
    ...
    try
        ...
        msgbox('Ошибка при построении линейной модели!');
    end
    ...
    if (size(k,2) == 6)
        ...
        msgbox('Не определены входные признаки!');
    end;
else
    if exist('k') == 1
    ...
    try
        ...
        msgbox('Ошибка при построении нелинейной модели!');
    end
...
if (tmp == 1)
    ...
    legend('Остатки от предсказанных');
else
    a1=scatter(r,y-r,5,[1 0 0]);
    title('Множественная нелинейная регрессия');
    legend('Остатки от предсказанных');
end
